...
const FileViewer = () => {
  ...
  const handleFileDelete = async (fileId: string) => {
    await fetch(`/api/assistants/files/${fileId}`, {
      method: "DELETE",
    });
  ...
  return (
    <div className={styles.fileViewer}>
      ...
      >
        {files.length === 0 ? (
          <div className={styles.title}>Attach legislation in pdf format</div>
        ) : (
          files.map((file) => (
      ...
      <div className={styles.fileUploadContainer}>
        ...
        <input
          ...
          multiple
          accept="application/pdf" /* Ensure only PDFs are allowed */
